...
#Function to print the top part of the html
def printTop():
...
if __name__ == "__main__":
  ...
  header1 = []
  htmlc1 = " "
  stu_data = OrderedDict() #it is used as a dictionary within a dictionary to story marks of each student
  ...
  # usn1 = "1SG15CS001"
  # usn2 = "1SG15CS005"
  # dip_usn1 = "1SG16CS401" #diploma usn
  # dip_usn2 = "1SG16CS402"
  ...
  if usn1[7] == '0':
    ...
    if usn1[8] == '0':
        z1 = '0'
        count = int(usn1[9]) #eg:- 1SG15CS001
    else:
        count = int(usn1[8:]) #eg:- 1SG15CS076
  else:
    count = int(usn1[7:]) #eg:- 1SG15CS125
  #end of if-else
  ...
  while count< (end_point+1) :
    ...
    #If the html page does not contain these codes, then that usn is invalid/does not exist or is in the previous year. Therefore, skip this usn
    if (html.find('15CS61')< 0 and html.find('15EE61') < 0 and html.find('15EC61') < 0  and html.find('15ME61') < 0 and html.find('15BT61') < 0 and html.find('15CV61') < 0) :
          count+=1
          #If the next usn belongs to diploma , the set the dip_flag and restart the count from the starting dip usn
  ...
  #findall returns a list.
            info = soup.findAll('div',{'class' : 'col-md-12 table-responsive'}) #get the top table that has the student info
            ...
            tds = soup.findAll('div', {'class': 'divTable'}) #get the second table that has the marks
            marks = tds[0].text
            marks1 = marks.strip()
            name = name_list[3]
            name1 = name[2:]
  ...
  #headers are removed
            ...
            #If the result is withheld, then in marks_list add an hyphen in place of External marks and total marks
            for i in range(0,48,6):
              if marks_list[i+4] == 'W': #check for "W"
                wt_flag = 1
  ...
  for i in range(0,len(header1)):
      if( header1[i].find('CSL') > 0): #If you find 'CSL', it is a lab subject. Hence credits=2
          sub_credit[header1[i]] = 2
      elif(len(header1[i]) == 7 and header1[i].find('MAT') <0 ): #electives
  ...
  for i in range (0,len(header1)):
    subject_tot[header1[i]] = 0 #total number of students in each subject
    subject_fail[header1[i]] = 0 #pass
    subject_wt[header1[i]] = 0#withheld
